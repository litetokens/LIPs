## Litetokens Wallet Proposal (LWP)
## Preamble

```
LIP: 0001
Title: Litetokens Wallet Proposal - Key Derivation Methods for Litetokens Accounts with BIP39
Author: getty.io
Created: 2017-04-05
```

## Simple Summary
This Litetokens Wallet Proposal describes methods for key derivation. This should improve key storage and moving keys between wallets and apps.

## Motivation
Currently the only way to store Litetokens account's keys is by saving a long str-encoded string which represents a secret key. However many industry standards have been developed for deriving keys which improve key storage and moving keys between wallets. Adapting these standards for Litetokens accounts should improve Litetokens wallets by:
